...
struct BarInfo {
    ...
    let aboutText = """
    Speaker Pub — место встречи с британским пивом и диджей-сетами.

    Внутри зала — темные деревянные панели и лаконичный дизайн. \
    Винтажная мебель, паркет и состаренная кирпичная кладка погружают \
    в атмосферу уютного места с историей.

    Провести приятный вечер можно за контактной барной стойкой, \
    пока бармены подбирают любимый сорт пива или миксуют коктейль.

    В теплое время года открываем для вас летнюю террасу.
    """
